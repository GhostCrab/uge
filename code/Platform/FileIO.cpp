...
bool FileExists(const wchar* filePath)
{
    ...
    DWORD fileAttr = GetFileAttributes((LPCSTR)filePath);
...
bool DirectoryExists(const wchar* dirPath)
{
    ...
    DWORD fileAttr = GetFileAttributes((LPCSTR)dirPath);
...
uint64 GetFileTimestamp(const wchar* filePath)
{
    ...
    Win32Call(GetFileAttributesEx((LPCSTR)filePath, GetFileExInfoStandard, &attributes));
...
void File::Open(const wchar* filePath, FileOpenMode openMode_)
{
    ...
    if(openMode == FileOpenMode::Read)
    {
        ...
        // Open the file
        fileHandle = CreateFile((LPCSTR)filePath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    else
    {
        ...
        if(FileExists(filePath))
            Win32Call(DeleteFile((LPCSTR)filePath));

        // Create the file
        fileHandle = CreateFile((LPCSTR)filePath, GENERIC_WRITE, 0, NULL, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, NULL);
        if(fileHandle == INVALID_HANDLE_VALUE)
        {
            std::wstring errPrefix = std::wstring(L"Failed to open file ") + filePath + L":\n";
            Assert_(false);
