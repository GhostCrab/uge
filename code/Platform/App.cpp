...

{
    ...
    GetModuleFileName(nullptr, (LPSTR)exePath, ArraySize_(exePath));
    const std::wstring exeDir = GetDirectoryFromFilePath(exePath);

    wchar expectedCurrDirectory[MAX_PATH] = { };
    GetFullPathName((LPCSTR)(exeDir + L"..\\..").c_str(), ArraySize_(expectedCurrDirectory), (LPSTR)expectedCurrDirectory, nullptr);

    SetCurrentDirectory((LPCSTR)expectedCurrDirectory);
