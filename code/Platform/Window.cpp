                                              ...
                                              hwnd(nullptr)
{
    ...
    if(accelResource)
    {
        accelTable = ::LoadAccelerators(hinstance, (LPCSTR)accelResource);
        if(!accelTable)
...
{
    ::DestroyWindow(hwnd);
    ::UnregisterClass((LPCSTR)appName.c_str(), hinstance);
}
...
void Window::MakeWindow(const wchar* sIconResource, const wchar* sSmallIconResource, const wchar* sMenuResource)
{
    ...
    HCURSOR hCursor = ::LoadCursorW(nullptr, (LPCWSTR)IDC_ARROW);
    ...
    WNDCLASSEX wc = {   sizeof(WNDCLASSEX),
                        ...
                        hCursor,
                        nullptr,
                        (LPCSTR)sMenuResource,
                        (LPCSTR)appName.c_str(),
                        hSmallIcon
                    };
...
{
    if(!::SetWindowText(hwnd, (LPCSTR)title))
        throw Win32Exception(::GetLastError());
}
...
{
    if(title == nullptr)
        return ::MessageBox(hwnd, (LPCSTR)message, (LPCSTR)appName.c_str(), type);
    else
        return ::MessageBox(hwnd, (LPCSTR)message, (LPCSTR)title, type);
}
